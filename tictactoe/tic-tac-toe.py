import numpy as np # to make easy changes to the board or matrix
import pygame   #to create the game window and draw the game elements
import math
import sys   #Used to safely exit the program when the game window is closed
Rows = 3
Columns = 3
sizeofsquare = 200
radiusofcircle = 60
offset = 55   #offset for the X figure, so that it fits well in the square, used for padding
circlelinewidth= 25
xlinewidth = 25
...
def printingboard():
	flippedboard = np.flip(Board, 0) #np.flip(array, axis) , axis=0 flips the array vertically
	print(flippedboard)
	print("")
...
def drawlines():
	pygame.draw.line(Screen, colorofline, (0, 200), (600, 200), 10) # 10 is the width of the line
